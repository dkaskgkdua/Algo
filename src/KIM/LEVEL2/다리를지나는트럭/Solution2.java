...
class Solution2 {
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        Queue<Integer> truckWeights = Arrays.stream(truck_weights)
                .boxed()
        ...
        while (!truckWeights.isEmpty()) {
            ...
            if (truckWeights.peek() + totalWeight > weight) {
                bridge.offer(0);
            } else {
                totalWeight += truckWeights.peek();
                bridge.offer(truckWeights.poll());
            }
        }
