...
public class Solution2 {
        ...
        while (true) {
            ...
            for (int r = 0; r < row - 1; r++) {
                for (int c = 0; c < col - 1; c++) {
                    Character block = Character.toLowerCase(listBoard.get(c).get(r));
                    ...
                    if (isMake(block, r, c, listBoard)) {
                        addTobeDEleted(listBoard, r, c, block);
                        isChanged = true;
                    }
                }
            }
            ...
            //제거
            for (List<Character> list : listBoard) {
                ...
                for (int temp = list.size(); temp < row; temp++) {
                    list.add(0, EMPTY_BLOCK);
                }
    ...
    private void addTobeDEleted(List<List<Character>> listBoard, int baseRow, int baseCol, Character block) {
        int[][] temps = new int[][]{{0, 0}, {0, 1}, {1, 0}, {1, 1}};

        for (int[] temp : temps) {
            listBoard.get(baseCol + temp[0]).set(baseRow + temp[1], block);
        }
    }

    private boolean isMake(Character block, int r, int c, List<List<Character>> listBoard) {
        int[][] temps = new int[][]{{0, 1}, {1, 0}, {1, 1}};

        for (int[] temp : temps) {
            if(!block.equals(Character.toLowerCase(listBoard.get(c + temp[0]).get(r + temp[1])))) {
                return false;
            }
        }
