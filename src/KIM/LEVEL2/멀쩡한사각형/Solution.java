...
public class Solution {
    public long solution(int w,int h) {
        long badSquares = 0;
        double previousYCoordinate = 0.0;
        ...
        for (int step = 1; step <= w ; step++) {
            double yCoordinate = h / (double)w * step;

            badSquares += (long)(Math.ceil(yCoordinate) - Math.floor(previousYCoordinate));

            if (yCoordinate == (double)(long)(yCoordinate)) {
                badSquares = badSquares * (w / step);
                break;
            }
