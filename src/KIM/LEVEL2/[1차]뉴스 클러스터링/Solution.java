package kr.co.programmers.kakao2018.lessons17677;
...
public class Solution {
    public int solution(String str1, String str2) {
        ...
        firstSet = convertToSet(str1.toLowerCase());
        secondSet = convertToSet(str2.toLowerCase());

        for (String element : secondSet.keySet()) {
            if (firstSet.containsKey(element)) {
                intersection += Math.min(firstSet.get(element), secondSet.get(element));
            }
        }

        union = getSetCount(firstSet) + getSetCount(secondSet) - intersection;

        return union == 0 ? 65536 : (int)((intersection / union) * 65536) ;
    }

    private int getSetCount(Map<String, Integer> set) {
        int temp = 0;
    ...
    private Map<String, Integer> convertToSet(String sentence) {
        Map<String, Integer> set = new HashMap<>();
        String element = "";

        for(int index = 0; index < sentence.length() - 1; index++) {
            if (Character.isAlphabetic(sentence.charAt(index)) && Character.isAlphabetic(sentence.charAt(index + 1))) {
                element = sentence.substring(index, index + 2);
